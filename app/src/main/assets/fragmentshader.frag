   ...
   for (int i = 0; i < NUMBER_OF_LIGHTS_SUPPORTED; i++) {
      ...
      if (light[i].position.w != zero_f) { // point light source
         ...
         if (light[i].spot_cutoff_angle < 180.0f) { // [0.0f, 90.0f] or 180.0f
            ...
            if (tmp_float >= cos(radians(light[i].spot_cutoff_angle))) {
                 if(blind_effect==1){
                 	tmp_float = pow(tmp_float, light[i].spot_exponent)*cos(90.0f*acos(tmp_float));
                 }
                 else{
      ...
      if (local_scale_factor > zero_f) {
         ...
         if(tmp_float > zero_f) {
                ...
                //modified code
                V_eye = -normalize(P_eye);
                tmp_float = dot(N_eye,L_eye);
                tmp_float = tmp_float + tmp_float;
                R_eye = tmp_float * N_eye - L_eye;
                tmp_float = dot(R_eye,V_eye);
...
#define FOG_NEAR_DISTANCE 50.0f
